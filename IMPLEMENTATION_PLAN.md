...
| Shemak 축 | 역할 | 소스 프로젝트 |
|-----------|------|--------------|
| **Pan HR** | 주기적 적정 인력 관리 | 인력관리 모듈 + 인력계획 모듈 + 스킬관리 모듈 |
...
| Components | shadcn/ui + Radix UI | 인력관리 모듈에서 이식 |
...
│   │
│   └── ui/                           # shadcn/ui (← 인력관리 모듈)
│
├── lib/
│   ├── db.ts                         # SQLite 연결 (← 인력관리 모듈 패턴)
...
| UI 기반 | shadcn/ui 셋업 | 인력관리 모듈 `components/ui/` 이식 |
...
| 작업 | 산출물 | 소스 |
|------|--------|------|
| 인력관리 | 조직 그리드, 효율 차트, 번아웃 경고 | 인력관리 모듈 핵심 이식 |
| 인력계획 | 예측 차트, FTE 게이지, 시나리오 카드 | 인력계획 모듈 패턴 → Recharts |
| 스킬관리 | 스킬 매트릭스, 갭 레이더 | 스킬관리 모듈 패턴 |
...
| 파일 | 용도 |
|------|------|
| 인력관리 모듈 `components/ui/` | shadcn/ui 컴포넌트 전체 |
| 인력관리 모듈 `components/dashboard/SummaryCards.tsx` | KPI 요약 카드 패턴 |
| 인력관리 모듈 `components/dashboard/CenterLevelGrid.tsx` | 조직 그리드 패턴 |
| 인력관리 모듈 `lib/db.ts` | DB 연결 패턴 |
